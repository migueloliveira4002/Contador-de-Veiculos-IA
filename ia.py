...
while True:
    ...
    for detection in results.pred[0]:
        ...
        if (label == 'car' and
            ...
            # Tentar associar com um carro existente
            car_matched = False
            ...
            # Se não encontrou correspondência, criar novo tracker
            if not car_matched:
                new_car = CarTracker(next_car_id, (center_x, center_y), frame_num)
                active_cars.append(new_car)
                next_car_id += 1

        # Desenhar retângulos e informações para todos os carros detectados
